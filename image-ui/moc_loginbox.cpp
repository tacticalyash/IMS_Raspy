/****************************************************************************
** Meta object code from reading C++ file 'loginbox.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.7.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "loginbox.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'loginbox.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.7.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_loginBox_t {
    QByteArrayData data[8];
    char stringdata0[90];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_loginBox_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_loginBox_t qt_meta_stringdata_loginBox = {
    {
QT_MOC_LITERAL(0, 0, 8), // "loginBox"
QT_MOC_LITERAL(1, 9, 18), // "loginButtonClicked"
QT_MOC_LITERAL(2, 28, 0), // ""
QT_MOC_LITERAL(3, 29, 13), // "authenticated"
QT_MOC_LITERAL(4, 43, 12), // "loginSuccess"
QT_MOC_LITERAL(5, 56, 2), // "go"
QT_MOC_LITERAL(6, 59, 8), // "indexOne"
QT_MOC_LITERAL(7, 68, 21) // "on_pushButton_clicked"

    },
    "loginBox\0loginButtonClicked\0\0authenticated\0"
    "loginSuccess\0go\0indexOne\0on_pushButton_clicked"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_loginBox[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       5,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   44,    2, 0x06 /* Public */,
       3,    0,   45,    2, 0x06 /* Public */,
       4,    0,   46,    2, 0x06 /* Public */,
       5,    0,   47,    2, 0x06 /* Public */,
       6,    0,   48,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       7,    0,   49,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,

       0        // eod
};

void loginBox::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        loginBox *_t = static_cast<loginBox *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->loginButtonClicked(); break;
        case 1: _t->authenticated(); break;
        case 2: _t->loginSuccess(); break;
        case 3: _t->go(); break;
        case 4: _t->indexOne(); break;
        case 5: _t->on_pushButton_clicked(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (loginBox::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&loginBox::loginButtonClicked)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (loginBox::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&loginBox::authenticated)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (loginBox::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&loginBox::loginSuccess)) {
                *result = 2;
                return;
            }
        }
        {
            typedef void (loginBox::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&loginBox::go)) {
                *result = 3;
                return;
            }
        }
        {
            typedef void (loginBox::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&loginBox::indexOne)) {
                *result = 4;
                return;
            }
        }
    }
    Q_UNUSED(_a);
}

const QMetaObject loginBox::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_loginBox.data,
      qt_meta_data_loginBox,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *loginBox::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *loginBox::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_loginBox.stringdata0))
        return static_cast<void*>(const_cast< loginBox*>(this));
    return QDialog::qt_metacast(_clname);
}

int loginBox::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}

// SIGNAL 0
void loginBox::loginButtonClicked()
{
    QMetaObject::activate(this, &staticMetaObject, 0, Q_NULLPTR);
}

// SIGNAL 1
void loginBox::authenticated()
{
    QMetaObject::activate(this, &staticMetaObject, 1, Q_NULLPTR);
}

// SIGNAL 2
void loginBox::loginSuccess()
{
    QMetaObject::activate(this, &staticMetaObject, 2, Q_NULLPTR);
}

// SIGNAL 3
void loginBox::go()
{
    QMetaObject::activate(this, &staticMetaObject, 3, Q_NULLPTR);
}

// SIGNAL 4
void loginBox::indexOne()
{
    QMetaObject::activate(this, &staticMetaObject, 4, Q_NULLPTR);
}
QT_END_MOC_NAMESPACE
